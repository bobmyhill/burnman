...
class HelperSolidSolution(burnman.Mineral):
    ...
    def __init__(self, endmembers, molar_amounts):
        """
        ...
        self.molar_amounts = molar_amounts
        assert(len(endmembers) == len(molar_amounts))
        assert(sum(molar_amounts) > 0.9999)
        assert(sum(molar_amounts) < 1.0001)
    ...
    def debug_print(self, indent=""):
        ...
        indent += "  "
        for (fraction, mat) in zip(self.molar_amounts, self.endmembers):
        ...
        for prop in self.endmembers[0].params:
            try:
                self.params[prop] = sum(
                    [self.endmembers[i].params[prop] * self.molar_amounts[i] for i in itrange])
            except TypeError:
...
class SLB_2005_ferropericlase(HelperSolidSolution):
    ...
    def __init__(self, fe_num):
        ...
        molar_amounts = [1. - fe_num, 0.0 + fe_num]
            # keep the 0.0 +, otherwise it is an array sometimes
        HelperSolidSolution.__init__(self, endmembers, molar_amounts)
...
class other_mg_fe_perovskite(HelperSolidSolution):
    ...
    def __init__(self, fe_num):
        ...
        molar_amounts = [1. - fe_num, 0.0 + fe_num] # keep the 0.0 +, otherwise it is an array sometimes
        HelperSolidSolution.__init__(self, endmembers, molar_amounts)
