...
P = 1.e5
temperatures = np.linspace(500., 1450., 101)
volumes = np.empty_like(temperatures)
for i, T in enumerate(temperatures):
    tro.set_state(P, T)
    volumes[i] = tro.V
...
data = listify_xy_file('data/V_troilite_Evans_et_al_2010.dat')
plt.plot(data[0], data[1]/1.e5, linestyle='None', marker='o')
...
# Set the standard state to be the same as VI
# That way, H_0, S_0, V_0, K_0, a_0 are easier to calculate
FeS_IV_HP.params['T_0'] = 300.
FeS_IV_HP.params['P_0'] = P_IV_VI

FeS_VI.set_state(P_IV_VI, T)
FeS_IV_HP.params['V_0'] = FeS_VI.V

# K (IV) = K (VI)
FeS_IV_HP.params['K_0'] = FeS_VI.params['K_0']

P_IV, T_IV, V_IV = zip(*data)
...
guesses = [FeS_VI.params['Kprime_0'], 6.e-5]  
print curve_fit(fit_PVT_data_Kprimea(FeS_IV_HP), PT_IV, V_IV, guesses)

FeS_VI.params['a_0'] = FeS_IV_HP.params['a_0']


pressures = np.linspace(1.e5, 250.e9, 101)
volumes_IV = np.empty_like(pressures)
volumes_VI = np.empty_like(pressures)
temperatures = [300., 1200., 2400.]
for T in temperatures:
    for i, P in enumerate(pressures):
        FeS_IV_HP.set_state(P, T)
        FeS_VI.set_state(P, T)
        volumes_IV[i] = FeS_IV_HP.V
        volumes_VI[i] = FeS_VI.V

    plt.plot(pressures/1.e9, volumes_IV, label='IV ('+str(T)+'K)')
    plt.plot(pressures/1.e9, volumes_VI, label='VI ('+str(T)+'K)')


plt.plot(P_VI/1.e9, V_VI, marker='o', linestyle='None', label='Ohfuji et al. (2007)')
plt.plot(P_IV/1.e9, V_IV, linestyle='None', marker='o', label='data')


plt.ylim(8.e-6, 16.e-6)
...
plt.plot(pressures/1.e9, volumes, label='tro')
...
T = 1200.
P_crossover = 18.e9
gibbs = find_gibbs(T, P_crossover, 55.e9, 0.5)
gibbs_array.append([P_crossover, T, gibbs])
...
plt.plot(pressures/1.e9, volumes, label='Tro')
plt.plot(pressures/1.e9, volumes_LP, label='dummy')
plt.plot(pressures/1.e9, volumes_HP, label='IV (HP)')
plt.title(str(T)+'K')
plt.legend(loc='upper right')
plt.show()
...
# First, however, we need to deal with Cp
# Let's assume that Cp is the same for FeS IV and metastable HP troilite

temperatures = np.linspace(500., 1400., 101)
Cps = np.empty_like(temperatures)

for i, T in enumerate(temperatures):
    tro.set_state(36.e9, T)
    Cps[i] = tro.C_p

guesses = tro.params['Cp']
print curve_fit(fitCp(tro), temperatures, Cps, guesses)

FeS_IV_HP.params['Cp'] = tro.params['Cp']
FeS_VI.params['Cp'] = tro.params['Cp']
tro.params['Cp'] = guesses


# Now we can solve for H and S
...
# Since IV and VI are at equilibrium at our standard state pressure
# at all temperatures, H_0 and S_0 are the same for both phases

FeS_VI.params['H_0'] = FeS_IV_HP.params['H_0']
FeS_VI.params['S_0'] = FeS_IV_HP.params['S_0']
FeS_VI.params['Cp'] = FeS_IV_HP.params['Cp']

print FeS_IV_HP.params
print FeS_VI.params

print fsolve(eqm_pressure([FeS_IV_HP, FeS_VI], [1.0, -1.0]), [30.e9], args=(1200.))


temperatures = np.linspace(500., 1400., 101)
Cps_tro = np.empty_like(temperatures)
Cps_IV_HP = np.empty_like(temperatures)
for i, T in enumerate(temperatures):
    tro.set_state(36.e9, T)
    FeS_IV_HP.set_state(36.e9, T)
    Cps_tro[i] = tro.C_p
    Cps_IV_HP[i] = FeS_IV_HP.C_p

plt.plot(temperatures, Cps_tro, 'b-')
plt.plot(temperatures, Cps_IV_HP, 'r--')
plt.show()
'''
