...
def create_minerals():
    ...
    # nagt properties from fitting of Dymshits et al. (2013, 2014) data,
    # modified for 50:50 py:namaj composition
    # Holland et al estimate is way too small

    # (at 50:50 py:namaj composition,
    # V = (11.42 A^3)/8*1e-30*6.0223e23 = 112.1 cm^3/mol)
    nagt.params['V_0'] = 112.1e-06
    ...
    plt.errorbar([0., 0., 0.0659, 0.1285, 0.1906],
                 np.array([1515.258, 1515.183, 1515.669, 1514.330, 1512.063])
                 * 1.e-30/8.*burnman.constants.Avogadro,
                 yerr=np.array([0.051, 0.088, 0.419, 0.309, 0.426])*1.e-30/8.
                 * burnman.constants.Avogadro, fmt='none', color='b')

    plt.scatter([0., 0., 0.0659, 0.1285, 0.1906],
                 np.array([1515.258, 1515.183, 1515.669, 1514.330, 1512.063])
                 * 1.e-30/8.*burnman.constants.Avogadro, color='b')
    ...
    fabdg = burnman.CombinedMineral([fefbdg, abdg],
                                    [0.5, 0.5],
                                    [-6.6e3, 0., 0.], name='fabdg')
    ...
    facor = burnman.CombinedMineral([cor, hem],
                                    [0.5, 0.5],
                                    [-6.6e3, 0., 0.], name='facor')
    fcor = burnman.CombinedMineral([mcor, mak, fak],
                                   [1., -1., 1.],
                                   [-15e3, 0., 0.], name='fcor')
    ...
    """
    # Entropies for fo-fa solid solutions from Dachs (2007).
    ...
    def func_Cp_mrw_Kojitani(T):
        return (164.30 + 1.0216e-2*T
                + 7.6665e3/T - 1.1595e7/T/T + 1.3807e9/T/T/T)

    def tweak(T):
        return 5./(2500.*2500.)*T*T

    T = np.linspace(306., 2500., 101)
    popt, pcov = curve_fit(func_Cp,
                           xdata=T,
                           ydata=func_Cp_mrw_Kojitani(T) + tweak(T),
                           p0=mrw.params['Cp'])
    mrw.params['Cp'] = popt

    # WARNING, THIS IS NOT THE CP OF MWD,
    # ITS JUST AN EASY WAY TO CREATE A SMOOTH MWD-MRW TRANSITION!!
    def tweak(T):
        return 2./(2500.*2500.)*T*T
    T = np.linspace(306., 2500., 101)
    popt, pcov = curve_fit(func_Cp,
                           xdata=T,
                           ydata=func_Cp_mrw_Kojitani(T) + tweak(T),
    ...
    # NOTE: making the heat capacity of frw lower than fa results in a
    # convex-down-pressure boundary in poor agreement
    # with the experimental data.
    # NOTE: making the heat capacity of frw the same as fa results
    # in a nearly-linear curve
    # NOTE: making the heat capacity of frw the same as
    # fa+10./(np.sqrt(2500.))*np.sqrt(T) results in a curve
    # that is too convex-up-pressure

    # NOTE: fwd should have a slightly lower heat capacity than frw
    def tweak(T):
        return -2./(np.sqrt(2500.))*np.sqrt(T)
    popt, pcov = curve_fit(func_Cp,
                           xdata=T,
                           ydata=np.array(Cp) + tweak(np.array(T)),
                           p0=fa.params['Cp'])
    fwd.params['Cp'] = popt
    ...
    ax[2].plot(temperatures, fo.evaluate(['C_p'], pressures, temperatures)[0],
               label='fo', linewidth=3.)
    ax[2].plot(temperatures, mwd.evaluate(['C_p'], pressures, temperatures)[0],
               label='mwd')
    ax[2].plot(temperatures, mrw.evaluate(['C_p'], pressures, temperatures)[0],
               label='mrw')
    ax[2].legend()

    #ax[3].imshow(fa_Cp_diagram, extent=[250., 2200., 100., 240.],
                  aspect='auto')
    ax[3].imshow(frw_Cp_diagram, extent=[0., 700., 0., 200.], aspect='auto')

    ax[3].plot(temperatures, fa.evaluate(['C_p'], pressures, temperatures)[0],
               label='fa', linewidth=3.)
    ax[3].plot(temperatures, fwd.evaluate(['C_p'], pressures, temperatures)[0],
               label='fwd')
    ax[3].plot(temperatures, frw.evaluate(['C_p'], pressures, temperatures)[0],
               label='frw')
    ax[3].legend()
    ...
    for name in solutions:
        create_polytope_attributes(solutions[name])
