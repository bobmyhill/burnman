...
def get_assemblages(mineral_dataset):
    ...
    for i, run in enumerate(set_runs):
        ...
        for j, chamber in enumerate(all_chambers[i]):
            ...
            print('WARNING, INCLUDING Re runs (maybe not at eqm?) but not including cpx')
            if (len(chamber_indices) > 1
                and (ds[run_indices[0]][5] == 'Fe'
                     or ds[run_indices[0]][5] == 'Mo'
                     or ds[run_indices[0]][5] == 'Re')):
                phases = []
                for idx in chamber_indices:
                    ...
                    # only include cpx for Fe-saturated or Mo-MoO2 runs
                    elif (phase_name == 'cpx'):
                        if (ds[run_indices[0]][5] == 'Fe'
                            or ds[run_indices[0]][5] == 'Mo'):
                            phases.append(solutions['cpx'])
                        else:
                            raise Exception('sorry, please comment out cpx from the Re-ReO2-buffered experiments in the dataset, as we do not know the Fe3+ content')
