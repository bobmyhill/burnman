...
for i, run_id in enumerate(set_runs):
    ...
    andr.params['formula'] = {'O':  12.0,
                              'Ca':  3.0,
                              'Fef': 2.0,
                              'Si':  3.0}
    aeg.params['formula'] = {'O':  6.0,
                             'Na':  1.0,
                             'Fef': 1.0,
                             'Si': 2.0}
    ...
    assemblage = burnman.Composite([gt, cpx, fcc_iron])
    ...
    # Process pyroxene
    cpx.fitted_elements = ['Na', 'Ca', 'Fe', 'Fef', 'Mg', 'Al', 'Si']
    cpx.composition = np.array([px_Na, px_Ca, px_Fe2, px_Fe3, px_Mg, px_Al, px_Si])
    cpx.compositional_uncertainties = np.identity(7)*(0.01 + cpx.composition*0.0005) # unknown errors
    ...
    cpx.compositional_uncertainties[2,2] = sig_prime[0][0]
    cpx.compositional_uncertainties[2,3] = sig_prime[0][1]
    cpx.compositional_uncertainties[3,2] = sig_prime[1][0]
    cpx.compositional_uncertainties[3,3] = sig_prime[1][1]
    
    burnman.processanalyses.compute_and_set_phase_compositions(assemblage)

    # py, alm, gt, andr, dmaj, nagt
    # di, hed, cen, cats, jd, aeg
    print(gt.composition)
    print(cpx.composition)
