...
x_Als = np.linspace(-0.5, 0.5, 3)
for x_Al in x_Als:

    f = 49.5/(49.5 + 5.2)
    composition = {'Si': 39.4 - x_Al,
                   'Al': 2.*2. + x_Al,
                   'Ca': 3.3,
                   'Mg': 49.5 + x_Al*f,
                   'Fe': 5.2 + x_Al*(1. - f),
    ...
    for sols in [sols_ol, sols_wad, sols_rw]:
        ...
        AlOct.extend([(c[0] + c[1] + c[2] + c[5]/2.)*2. for c in c_gt])
        FeOct.extend([c[3]*2. for c in c_gt])
    ...
    ax[0].plot(pressures/1.e9, Fe3, label='{0}'.format(composition['Al']))
    ax[1].plot(pressures/1.e9, FeOct/AlOct, label='{0}'.format(composition['Al']))
...
#plt.ylim(0.,0.6)
plt.savefig('gt_MgAl_NaSi_Fe_saturated.pdf')
plt.show()
