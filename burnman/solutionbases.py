...
class gridded_simplex(object):
    ...
    def generate(self, generate_type='list'):
        ...
        if generate_type == 'list':
            ...
            raise Exception('generate should create either '
...
class SolutionPolytope(object):
    def __init__(self, equalities, return_fractions=False):
        ...
        positivity_constraints = np.concatenate((np.zeros((len(equalities[0])
                                                           - 1, 1)),
                                                 np.identity(len(equalities[0])
                                                             - 1)), axis=1)
    ...
    def endmember_occupancies(self):
        if self.return_fractions:
            v = np.asarray(self.raw_vertices, dtype=Fraction)
        else:
...
def feasible_solution_basis_in_component_space(solution, components):
    """
    Note that this function finds the extreme endmembers
    and finds the subset within the components.
    Thus, starting with a solution with a disordered endmember
    and then restricting component range may
    produce a smaller solution than intended.
    ...
    component_array, component_elements = compositional_array([dictionarize_formula(c)
                                                               for c in components])
    ...
    # 4) Find the endmembers that can be described with a
    # linear combination of components

    # 4a) First, add elements to endmember_elements which
    # are in component_elements
...
def _subregular_matrix_conversion(new_basis, binary_matrix,
    ...
    # array needs to be copied, otherwise just a view onto Wn
    new_endmember_excesses = np.copy(np.einsum('iii->i', Wn))

    # Removal of endmember components from 3D representation
    Wn -= (np.einsum('i, j, k->ijk', new_endmember_excesses,
                     np.ones(n_mbrs), np.ones(n_mbrs))
           + np.einsum('i, j, k->ijk', np.ones(n_mbrs),
                       new_endmember_excesses, np.ones(n_mbrs))
           + np.einsum('i, j, k->ijk', np.ones(n_mbrs),
                       np.ones(n_mbrs), new_endmember_excesses))/3.
...
def transform_solution_to_new_basis(solution, new_basis, n_mbrs=None,
    ...
    if solution_type == 'ideal':
        ...
        raise Exception('The function to change basis for the '
                        '{0} solution model has not '
                        'yet been implemented.'.format(solution_type))
...
def feasible_solution_in_component_space(solution, components,
    ...
    Thus, starting with a solution with a disordered endmember
    and then restricting component range
    may produce a smaller solution than intended.
    For example, with the endmembers [A] and [A1/2B1/2],
    the extreme endmembers are [A] and [B].
    A component space A--AB will result in only endmember [A] being valid!!
    """
