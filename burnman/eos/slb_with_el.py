...
class SLBELBase(eos.EquationOfState):
    """
    Base class for a modified version of the finite strain-Mie-Grueneiesen-Debye 
    equation of state detailed in :cite:`Stixrude2005`.  
    For the most part the equations are all third order in strain, but see further 
    the :class:`burnman.slbel.SLBEL2` and :class:`burnman.slbel.SLBEL3` classes.

    This base class additionally has an electronic contributions to the free energy, 
    to deal with very high temperatures or minerals such as the iron polymorphs where
    the electronic contribution to the heat capacity is significant even at low temperatures.
    """
    ...
    def __thermal_pressure(self,T,V, params):
        ...
        P_th = gr * ( debye.thermal_energy(T, Debye_T, params['n']) \
                      + electronic.thermal_energy(T, params['V_0']/V,
                                                  params['T_el'], params['Cv_el']))/V 
        return P_th
    ...
    def volume(self, pressure, temperature, params):
        ...
        E_th =  lambda x : debye.thermal_energy(temperature, debye_T(x), params['n']) \
                + electronic.thermal_energy(temperature, params['V_0']/x,
                                            params['T_el'], params['Cv_el']) #thermal energy at temperature T
        E_th_ref = lambda x : debye.thermal_energy(T_0, debye_T(x), params['n']) \
                   + electronic.thermal_energy(T_0, params['V_0']/x,
                                               params['T_el'], params['Cv_el']) #thermal energy at reference temperature
    ...
    def pressure( self, temperature, volume, params):
        ...
        dV = 1.e-9
        F0 = self.helmholtz_free_energy(0., temperature, volume-0.5*dV, params)
        F1 = self.helmholtz_free_energy(0., temperature, volume+0.5*dV, params)
        P = (F0 - F1)/dV
        ...
        E_th = debye.thermal_energy(temperature, debye_T, params['n']) \
               + electronic.thermal_energy(temperature, params['V_0']/volume,
                                           params['T_el'], params['Cv_el'])
        E_th_ref = debye.thermal_energy(T_0, debye_T, params['n']) \
                   + electronic.thermal_energy(T_0, params['V_0']/volume,
                                               params['T_el'], params['Cv_el']) #thermal energy at reference temperature
    ...
    def isothermal_bulk_modulus(self, pressure,temperature, volume, params):
        ...
        dV = 1.e-9
        F0 = self.helmholtz_free_energy(pressure, temperature, volume - dV, params)
        F1 = self.helmholtz_free_energy(pressure, temperature, volume + dV, params)
        F = self.helmholtz_free_energy(pressure, temperature, volume, params)

        return volume*((F1 - F)/dV - (F-F0)/dV)/dV
        ...
        C_v = debye.heat_capacity_v(temperature, debye_T, params['n']) \
              + electronic.heat_capacity_v(temperature, params['V_0']/volume,
                                           params['T_el'], params['Cv_el']) #heat capacity at temperature T
        C_v_ref = debye.heat_capacity_v(T_0,debye_T, params['n']) \
                  + electronic.heat_capacity_v(T_0, params['V_0']/volume,
                                               params['T_el'], params['Cv_el']) #heat capacity at reference temperature
    ...
    def thermal_expansivity(self, pressure, temperature, volume, params):
        ...
        dV = 1.e-9
        dT = 1.
        F0 = self.helmholtz_free_energy(pressure, temperature-0.5*dT, volume-0.5*dV, params)
        F1 = self.helmholtz_free_energy(pressure, temperature+0.5*dT, volume-0.5*dV, params)
        F2 = self.helmholtz_free_energy(pressure, temperature-0.5*dT, volume+0.5*dV, params)
        F3 = self.helmholtz_free_energy(pressure, temperature+0.5*dT, volume+0.5*dV, params)
        dFdT0 = (F1 - F0)/dT
        dFdT1 = (F3 - F2)/dT
        K = self.isothermal_bulk_modulus(pressure, temperature, volume, params)
        return -(dFdT1 - dFdT0)/dV/K
        '''
        print 'HERE'
        C_v = self.heat_capacity_v(pressure, temperature, volume, params)
        gr = self.grueneisen_parameter(pressure, temperature, volume, params)
    ...
    def entropy( self, pressure, temperature, volume, params):
        ...
        S = debye.entropy(temperature, Debye_T, params['n'] ) \
            + electronic.entropy(temperature, params['V_0']/volume,
                                 params['T_el'], params['Cv_el'])
        return S 
    ...
    def helmholtz_free_energy( self, pressure, temperature, volume, params):
        ...
        F_quasiharmonic = (debye.helmholtz_free_energy(temperature, Debye_T, params['n'] ) \
                           + electronic.helmholtz_free_energy(temperature, params['V_0']/volume,
                                                              params['T_el'], params['Cv_el'])) \
            - (debye.helmholtz_free_energy(T_0, Debye_T, params['n'] ) \
               + electronic.helmholtz_free_energy(T_0, params['V_0']/volume,
                                                  params['T_el'], params['Cv_el']))
    ...
    def validate_parameters(self, params):
        ...
        if 'T_el' not in params:
            params['T_el'] = 100000.

        if 'Cv_el' not in params:
            params['Cv_el'] = 0.
