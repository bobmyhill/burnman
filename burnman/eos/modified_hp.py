...
class MOD_HP_TMT(eos.EquationOfState):
    ...
    def gibbs_free_energy(self, pressure, temperature, volume, params):
        ...
        return (
            ...
            - temperature
            * (params["S_Pref"] + self.__intCpoverTdT(temperature, params))
            + intVdP
        )
    ...
    def validate_parameters(self, params):
        ...
        if "T_einstein" not in params:
            params["T_einstein"] = 10636.0 / (params["S_Pref"] / params["n"] + 6.44)
        ...
        # no test for H_0
        if params["S_Pref"] is not float("nan") and params["S_Pref"] < 0.0:
            warnings.warn("Unusual value for S_Pref", stacklevel=2)
        if params["V_0"] < 1.0e-7 or params["V_0"] > 1.0e-2:
            warnings.warn("Unusual value for V_0", stacklevel=2)

        if self.molar_heat_capacity_pref(params["T_0"], params) < 0.0:
            warnings.warn("Negative heat capacity at T_0", stacklevel=2)
        if self.molar_heat_capacity_pref(2000.0, params) < 0.0:
