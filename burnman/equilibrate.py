...
def calculate_baseline_endmember_amounts(assemblage, equality_constraints, prm):
    ...
    if len(reduced_nullspace) != 0:
        ...
        baseline_endmember_amounts = np.array(Rinv).dot(baseline_proportions)

        cons_fun = lambda x:  np.array(R).dot(baseline_endmember_amounts + reaction_vectors.dot(x)) # >= 0
        ...
        baseline_endmember_amounts += np.array(reaction_vectors).dot(sol.x)
    else:
        baseline_endmember_amounts = np.array(Rinv).dot(baseline_proportions)

    # Get compositional residuals
    res = np.array(prm.stoichiometric_matrix).dot(baseline_endmember_amounts) - prm.bulk_composition_vector
    try:
        if any(res > 1.e-9):
            print('Residuals:\n{0}'.format(res))
...
def parameter_constraints_to_endmember_constraint_vector(constraint, indices):
    """
    Unlike the parameter constraints,
    the endmember constraints Ax=b should always have b=0
    """
    values = []
    for i, start_idx in enumerate(phase_proportion_parameter_indices(indices)):
        n_mbrs = len(indices[i])
        values.extend([nsimplify(constraint[0][start_idx]
                                 + constraint[1])] * n_mbrs)
...
def set_compositions_and_state_from_parameters(x, assemblage, indices,
                                               endmembers_per_phase):
...
def F(x, assemblage, equality_constraints, prm):
    ...
    set_compositions_and_state_from_parameters(x, assemblage, prm.indices,
                                               prm.endmembers_per_phase)
    ...
    for i, mbr_indices in enumerate(prm.indices):
        n = len(mbr_indices)
        if mbr_indices[0] is None:  # for endmembers
    ...
    eqns[2+n_reactions:] = np.dot(prm.stoic_colspace,
                                  (new_endmember_amounts
                                   - prm.baseline_endmember_amounts))
...
def jacobian(x, assemblage, equality_constraints, prm):
    ...
    # on the bulk compositional constraints
    ...
    for i, mbr_indices in enumerate(prm.indices):
        n = len(mbr_indices)
        if mbr_indices[0] is not None:  # ignore pure phases, as changing proportions of a phase does not change its molar gibbs free energy
    ...
    # np.einsum('ij, i->ij', full_hessian,
    # scaling(equality_constraints, n_indices, n_reactions))
    return full_hessian
...
def phase_composition_constraint(phase, assemblage, indices, constraint):
    ...
    # +3 comes from P, T and the proportion of the phase of interest
    start_idx = int(sum([len(i) for i in indices[:phase_idx]])) + 3
    n_indices = sum([len(i) for i in indices])
...
def equilibrate(composition, assemblage, equality_constraints,
    ...
    # Annoyingly, python overwrites default values and stores them
    # for the next function run
    # Thus, we need to have the following two lines to set a
    # fixed default initial_state, rather than setting a
    # default in the function call above.
    if initial_state is None:
        initial_state = [5.e9, 1000.]

    # First, overwrite initial state if the user wants
    # it set from the assemblage
    ...
    for i in range(2):
        if equality_constraints[i][0] == 'phase_proportion':
            ...
            if not isinstance(proportion, np.ndarray):
                raise Exception(f'The constraint proportion in equality {i+1} '
                                'should be a float or numpy array')
            ...
            if not isinstance(constraint[3], np.ndarray):
                raise Exception('The last constraint parameter in equality '
                                f'{i+1} should be a float or numpy array')
            ...
            raise Exception('The type of equality_constraint is '
                            f'not recognised for constraint {i+1}.\n'
                            'Should be one of P, T, S, V, X,\n'
                            'PT_ellipse, phase_proportion, '
                            'or phase_composition.')
    ...
    # Reformat equality constraints if they are
    # related to pressure or temperature
    for i in range(2):
    ...
    if initial_composition_from_assemblage:
        prm.baseline_endmember_amounts = get_endmember_amounts(assemblage,
                                                               prm.indices)
    ...
    for i_c0 in range(n_c0):
        ...
        for i_c1 in range(n_c1):
            ...
            equality_constraints = [equality_constraint_lists[0][i_c0],
                                    equality_constraint_lists[1][i_c1]]

            # Set the initial proportions and compositions
            # of the phases in the assemblage:
            set_compositions_and_state_from_parameters(prm.initial_parameters,
                                                       assemblage,
                                                       prm.indices,
                                                       prm.endmembers_per_phase)
            ...
            if (sol.success is False and exit_on_first_failure):
...
def equilibrium_order_init(solution):
    ...
    """
    assemblage = Composite([solution], [1.])
    ...
    sol, prm = equilibrate(solution.formula, assemblage,
