...
if __name__ == "__main__":
    ...
    class SpinEOS(object):
        def __init__(self, params):
            ...
            self.delFVtr = (self.params['HP phase'].method.molar_internal_energy(pressure = 0.,
                            ...
                            - self.params['LP phase'].method.molar_internal_energy(pressure = 0.,
                                                                                   temperature = self.params['T_0'],
                                                                                   volume = self.V_tr,
                                                                                   params = self.params['LP phase'].params))/burnman.constants.Avogadro*self.params['Z']
            self.delFVtr -= (burnman.constants.Boltzmann
                             * self.params['T_0']
        ...
        def Z(self, volume, temperature):
            ...
            beta = 1./(burnman.constants.Boltzmann*temperature)
    ...
    ax[0].scatter(dat[:, 0], dat[:, 1]*6.022/4./10.)
    dat = np.loadtxt('fp35_vlh_2000K.dat')
    ...
    for T in [300., 2000.]:
        ...
        #ax[0].scatter(dat[:,0], dat[:,1]*6.022/4./10.)
        #dat = np.loadtxt('fp35_vll_2000K.dat')
        #ax[0].scatter(dat[:,0], dat[:,1]*6.022/4./10.)
        ...
        ax[0].plot(pressures/1.e9, volumes3*1.e6, linestyle='--')
