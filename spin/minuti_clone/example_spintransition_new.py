...
if __name__ == "__main__":
    ...
    class SpinEOS(object):
        ...
        def omega(self, volume, temperature):
                ...
                return (delFV /(self.params['n'] * (self.e_HP - self.e_LP)))
    ...
    params = {'LP phase': fper_HS(),
              ...
              'spin state energies': np.array([0., 0.5, 1.]),  # LS, IS, HS
              'orbital degeneracies': np.array([1., 6., 3.]),
              'unpaired electrons': np.array([0., 2., 4.])}
    """
    # Only allow HS and LS states
    params = {'LP phase': fper_HS(),
              'HP phase': fper_LS(),
              'n': 0.35,
              'LP index': 1,
              'HP index': 0,
              'spin state names': ['LS', 'HS'],
              'spin state energies': np.array([0., 1.]),  # LS, HS
              'orbital degeneracies': np.array([1., 1.]),
              'unpaired electrons': np.array([0., 4.])}
    ...
    prps = np.empty((501, len(fper.params['spin state energies'])))
    ...
    for T in [300., 800., 1300., 1800.]:
        ...
        #for i in range(len(fper.params['spin state energies'])):
        #ax[1].plot(pressures/1.e9, prps[:, 0], label='LS')
        ax[1].plot(pressures/1.e9, prps[:, 1], label=f'{fper.params["spin state names"][1]} at {T} K')
