...
initial_enthalpy_interaction=[[0.e3, 0.0e3], [0.e3]]
###########################
...
'''
Model for wustite for a single temperature (Margules solution model)
'''
...
T=1184.733
fcc.set_state(1.e5, 1184.733)
print gibbs_fcc_1bar(T), fcc.gibbs, gibbs_bcc_1bar(T)
...
def margules_excess(Xj, Wij, Wji, Ws, T):
    return (1.-Xj)*Xj*((Wji+Ws*T)*(1.-Xj) + (Wij+Ws*T)*Xj)
...
def fO2_wus_eqm_Fe(data, Wij, Wji, Ws, H_0_FeO, H_0_Fe34O, S_0_Fe34O): # last fO2 should be zero
    ...
    for datum in data:
        ...
        wus.set_composition([0., 1-3.*(y-delta), 3.*(y-delta)])
        wus.set_state(Pr, T)
        gibbs0=wus.gibbs + margules_excess(3.*(y-delta), Wij, Wji, Ws, T)

        wus.set_composition([0., 1-3.*(y+delta), 3.*(y+delta)])
        wus.set_state(Pr, T)
        gibbs1=wus.gibbs + margules_excess(3.*(y+delta), Wij, Wji, Ws, T)

        mu_O2=2*((1.-y+delta)/(1.-y-delta)*gibbs1 - gibbs0)/((1.-y+delta)/(1.-y-delta)-1.)

        oxygen.set_state(Pr, T)
        fO2_wus=np.log10(np.exp((mu_O2-oxygen.gibbs)/(constants.gas_constant*T)))

        wus.set_composition([0., 1-3.*y, 3.*y])
        wus.set_state(Pr, T)
        phase.set_state(Pr, T)
        gibbs_wus=wus.gibbs + margules_excess(3.*y, Wij, Wji, Ws, T)

        if phase == wus:
            fO2.append(fO2_wus)
        elif phase == mt:
            gibbs_mt=phase.gibbs # MT
...
guesses=[14.e3, 14.e3, 0., -429440., -450000., 60.]
...
def bcc_eqm(comp, P, T):
    ...
    wus.set_composition([0., 1-c, c])
    wus.set_state(P, T)
    mu_O2=2*((1.-y+delta)/(1.-y-delta)*gibbs1 - gibbs0)/((1.-y+delta)/(1.-y-delta)-1.)
    mu_iron = (wus.gibbs + margules_excess(3.*y, Wij, Wji, Ws, T) - 0.5*mu_O2)/(1.-y) 
...
def fcc_eqm(comp, P, T):
    ...
    #fcc.set_state(P, T)
    return  mu_iron-gibbs_fcc_1bar(T)
...
def mt_eqm(comp, P, T):
    ...
    mu_mt=3.*mu_iron + 2.*mu_O2
    mt.set_state(P,T)
    return mu_mt - mt.gibbs # MT

# Function to return the equilibrium temperature between bcc iron, wustite and magnetite
def iron_wus_mt_eqm(T, P):
    diff_comp=optimize.fsolve(bcc_eqm, 0.16, args=(P, T))[0] - optimize.fsolve(mt_eqm, 0.16, args=(P, T))[0]
    print diff_comp
    return diff_comp
