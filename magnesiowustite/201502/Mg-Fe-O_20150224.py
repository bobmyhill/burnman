...
class wuestite_ss(burnman.SolidSolution):
    def __init__(self):
        ...
        endmembers = [[minerals.HP_2011_ds62.per(), '[Mg]O'],[wustite(), '[Fe]O'],[fe34o(), '[Fe1/4Fef1/2Vc1/4]O']]
...
def fO2_wus_eqm_Fe(data, Wij, Sij, H_0_FeO, H_0_Fe34O, S_0_Fe34O): # last fO2 should be zero
    ...
    for datum in data:
        phase, T, y = datum
        wus.set_composition([0., 1-4.*y, 4.*y])
        wus.set_state(Pr, T)
...
def fO2_wus_model(ys, T):
    ...
    for y in ys:
        ...
        wus.set_composition([0.0, 1-4.*y, 4.*y])
        wus.set_state(Pr, T)
        fO2.append(np.log10(fugacity(oxygen, [wus])))
    return fO2
...
wus.set_composition([0., 1.-0.075*4., 0.075*4.])
