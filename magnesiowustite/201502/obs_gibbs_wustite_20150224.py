...
log10fO2_Fe=quadratic(y_Fe, a, b, c)
log10fO2_Fe3O4=quadratic(y_Fe3O4, a, b, c)
...
gibbs_mt=gibbs_mt_ONeill(T) + (3.*fcc.gibbs + 2.*oxygen.gibbs)
print gibbs_mt_ONeill(T), gibbs_mt
...
gibbs_FeO_Fe3O4_ONeill = (1.-y_Fe3O4)*(gibbs_mt/3. - 2./3.*mu_O2_Fe3O4) + 0.5*mu_O2_Fe3O4
...
for i in range(len(ys)-1):
    y_mid=(ys[i] + ys[i+1])/2.
    log10fO2_FeO=quadratic(y_mid, a, b, c)
...
for i in range(len(ys)):
    delta=0.000001
    a, b, c = popt_quadratic
    gibbs_fit[i]= quadratic(ys[i], a, b, c) - T*entropy(ys[i])
    ...
    a, b, c, d = popt_cubic
    gibbs_fit[i]= cubic(ys[i], a, b, c, d) - T*entropy(ys[i])
    ...
    a, b, c, d, e = popt_quartic
    gibbs_fit[i]= quartic(ys[i], a, b, c, d, e) - T*entropy(ys[i])
