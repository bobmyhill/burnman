...
def _atomic_momenta(temperature, volume, params): # F_ig, eq. S6
    ...
    for element, N in params['formula'].iteritems():
        #print element, N
        q=volume*_partition_function(atomic_masses[element])
        entropy_sum+=N*(1. + np.log(q/N)) # eq. 16.72 of Callen., 1985; p. 373
    return -constants.Boltzmann*temperature*entropy_sum
...
def _zeta(temperature, volume, params): # eq. S5a
...
class SiO2_liquid():
    params={'formula': {'Si': 1., 'O': 2.},
                 'V_0': 0.2780000000E+02, # F_cl (1,1)
                 'T_0': 0.3000000000E+04, # F_cl (1,2)
                 'E_0': -.2360007614E+04, # F_cl (1,3)
                 'S_0': -.1380253514E+00, # F_cl (1,4)
                 'O_f':2, # F_cl (3)
                 'm':0.91, # F_cl (5)
                 'O_theta':5,
...
temperature=3000.
volume=0.2780000000E+02
print 'Helmholtz:', helmholtz_free_energy(temperature, volume, SiO2_liq.params)
print 'F_xs:', _bonding_energy(temperature, volume, SiO2_liq.params)
print 'F_el:', _electronic_excitation_energy(temperature, volume, SiO2_liq.params)
print 'F_ig:', _atomic_momenta(temperature, volume, SiO2_liq.params)
