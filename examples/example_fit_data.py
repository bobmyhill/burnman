...
if __name__ == "__main__":
    ...
    PTp_sigmas = np.empty_like(PTp_data)
    PTp_sigmas[:, 0] = PTp_data[:, 0]*0.1  # 10% of total pressure
    PTp_sigmas[:, 1] = PTp_data[:, 1]*0.0001  # 0.01% of total temperature
    PTp_sigmas[:, 2] = PTp_data[:, 2]*0.005  # 0.5% of total Vs
    ...
    def best_fit():
        return burnman.eos_fitting.fit_PTp_data(mineral = mg_perovskite_test,
                                                ...
                                                data = PTp_data,
                                                data_covariances = np.power(PTp_sigmas, 2.),
                                                verbose = False)
    ...
    plt.scatter(fitted_eos.data_mle[:, 0] / 1.e9, fitted_eos.data_mle[:, 2] / 1.e3)
    plt.ylim([6.55, 8])
    plt.xlim([25., 135.])
    ...
    nul = TH_data[:,0]*0.
    PTH_covariances = np.array([[TH_data[:,0]*0. + 1.e0, nul, nul],
                                [nul, TH_data[:,1], nul],
                                [nul, nul, np.power(TH_data[:,2]*4.184*0.0004, 2.)]]).T
