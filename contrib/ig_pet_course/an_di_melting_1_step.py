...
for j, T_C in enumerate(T_Cs):
    ...
    ln = ax[1].plot(xs, gibbs[j]/1.e3, label=f'T: {T_Cs[j]} C',
                    linestyle=':')
    ln = axsp[1].plot(xs, gibbs[j]/1.e3,
                      linestyle=':', label='$\mathcal{G}$ (melt-solid)')
    xs_min = []
    gibbs_min = []
    entropy_min = []
    enthalpy_min = []
    ...
    axsp[1].plot(xs_min, np.array(gibbs_min)/1.e3, color=ln[0].get_color(), label='$\mathcal{G}$ (eqm)')
    try:
    ...
    if xs_eqm[j, 0] > 0.0001 and gibbs_eqm[j, 0] < 0.:
        ...
        axsp[1].scatter([xs_eqm[j, 0]], [gibbs_eqm[j, 0]/1.e3],
                        color=ln[0].get_color(), label='$\mathcal{G}$ (liq)')
    ...
    axsp[1].scatter([1.], [0.], c='purple', label='$\mathcal{G}$ (an)')
    axsp[1].scatter([0.], [0.], c='orange', label='$\mathcal{G}$ (di)')
    axsp[1].set_ylim(-10, 10)

    axsp[1].legend()
    figsp.savefig(f'figures/an_di_melting_simple_step_{j}.pdf')
