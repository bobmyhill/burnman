...
print('Path 2')
assemblage = burnman.Composite([fo], [1.], 'molar', 'forsterite')

pressures_2b = np.linspace(1.e5, pressures_1[-1], 501)
equality_constraints = [['P', pressures_2b],
                        ['S', entropies_1[-1]]]
...
pressures_2a = temperatures_2a * 0. + 1.e5
...
dS_2 = np.concatenate((np.gradient(entropies_2a, edge_order=2), np.gradient(entropies_2b, edge_order=2)))
dV_2 = np.concatenate((np.gradient(volumes_2a, edge_order=2), np.gradient(volumes_2b, edge_order=2)))


# Path 3: Isentropic then isobaric
print('Path 3')
pressures_3a = np.linspace(1.e5, pressures_1[-1], 501)[:-1]
equality_constraints = [['P', pressures_3a],
                        ['S', entropies_1[0]]]
sols = equilibrate(assemblage.formula, assemblage, equality_constraints)
...
pressures_3b = temperatures_3b * 0. + pressures_1[-1]
...
dS_3 = np.concatenate((np.gradient(entropies_3a, edge_order=2), np.gradient(entropies_3b, edge_order=2)))
dV_3 = np.concatenate((np.gradient(volumes_3a, edge_order=2), np.gradient(volumes_3b, edge_order=2)))


fig = plt.figure(figsize=(9, 3))
ax = [fig.add_subplot(1, 3, i) for i in range(1, 4)]

Q_1 = cumulative_trapezoid(temperatures_1*dS_1, initial=0.)
Q_2 = cumulative_trapezoid(temperatures_2*dS_2, initial=0.)
Q_3 = cumulative_trapezoid(temperatures_3*dS_3, initial=0.)


W_1 = cumulative_trapezoid(pressures_1*dV_1, initial=0.)
W_2 = cumulative_trapezoid(pressures_2*dV_2, initial=0.)
W_3 = cumulative_trapezoid(pressures_3*dV_3, initial=0.)


ax[0].plot(temperatures_3, pressures_3/1.e9, label='Path 3', linestyle=':')
ax[0].plot(temperatures_2, pressures_2/1.e9, label='Path 2', linestyle='--')
ax[0].plot(temperatures_1, pressures_1/1.e9, label='Path 1')

ax[1].plot(W_3/1000., Q_3/1000., label='Path 3', linestyle=':')
ax[1].plot(W_2/1000., Q_2/1000., label='Path 2', linestyle='--')
ax[1].plot(W_1/1000., Q_1/1000., label='Path 1')
...
ax[2].plot(temperatures_3, (entropies_3 - entropies_3[0]), label='Path 3', linestyle=':')
ax[2].plot(temperatures_2, (entropies_2 - entropies_2[0]), label='Path 2', linestyle='--')
