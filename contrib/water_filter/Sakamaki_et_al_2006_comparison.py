...
melt = melt_all_liquid([0.8, 0.2, 0.])
molar_mass_sol = melt.molar_mass
melt = melt_all_liquid([0., 0., 1.])
molar_mass_H2O = melt.molar_mass

fig = plt.figure(figsize=(14,6))
ax = [fig.add_subplot(1, 2, i) for i in range(1, 3)]
Fig2_img = mpimg.imread('data/Sakamaki_et_al_2006_Fig_2.png')
ax[0].imshow(Fig2_img, extent=[0.0, 20.0, 0., 50.], aspect='auto')

T = 2473.15
pressures = np.linspace(1.5e9, 22.e9, 101)
temperatures = pressures*0. + T

melt = melt_all_liquid([0., 0., 1.])
volumes = melt.evaluate(['V'], pressures, temperatures)[0]

ax[0].plot(pressures/1.e9, volumes*1.e6, label=f'Pure H$_2$O ({T} K; Sterner and Pitzer, 1994)')
ax[0].plot((pressures - 1.5e9)/1.e9, volumes*1.e6 - 2.5, label=f'Pure H$_2$O ({T} K; shifted)')


Fig3b_img = mpimg.imread('data/Sakamaki_et_al_2006_hydrous_umafic_melt_1873K.png')
ax[1].imshow(Fig3b_img, extent=[0.0, 20.0, 2500., 4000.], aspect='auto')
...
for mass_H2O in [0.00, 0.02, 0.04, 0.06, 0.08]:
    ...
    pressures = np.linspace(6.e9, 20.e9, 101)
    temperatures = pressures*0. + 1873.

    densities = melt.evaluate(['rho'], pressures, temperatures)[0]


    ax[1].plot(pressures/1.e9, densities, label=f'This study, {mass_H2O*100} wt% H$_2$O (P+S 1994)')
ax[0].legend()
ax[1].legend()
