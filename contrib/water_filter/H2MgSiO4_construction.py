...
print(
    "If we use any of these, we must change HSC->SUP convention for Holland and Powell "
    "endmembers to make consistent with SLB database"
)

Mg2SiO4 = SLB_2011.forsterite()
H2MgSiO4 = burnman.CombinedMineral(
    [HP_2011_ds62.phA(), SLB_2011.mg_wadsleyite(), SLB_2011.hp_clinoenstatite()],
    [1.0 / 3.0, -5.0 / 3.0, 1.0],
)
...
H2MgSiO4.set_state(1.0e5, 298.15)
print(H2MgSiO4.V)
print(H2MgSiO4.S)
print(H2MgSiO4.formula)
print(H2MgSiO4.K_T / 1.0e9)
K1 = H2MgSiO4.K_T

dP = 1000.0
H2MgSiO4.set_state(1.0e5 + dP, 298.15)
K2 = H2MgSiO4.K_T
...
pressures = np.linspace(1.0e9, 60.0e9, 101)
for T in [300.0, 2000.0]:
    ...
    ax[0].plot(pressures / 1.0e9, Vs1, label="Mg2SiO4")
    ax[0].plot(pressures / 1.0e9, Vs2, label="H2MgSiO4")
    ax[1].plot(pressures / 1.0e9, Ss1, label="Mg2SiO4")
    ax[1].plot(pressures / 1.0e9, Ss2, label="H2MgSiO4")
plt.legend()
plt.show()
