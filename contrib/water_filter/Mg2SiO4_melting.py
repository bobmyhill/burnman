...
wad_ring_lm = inv([wad, ring, lm], [1.0, 1.0, 2.0])
wad_lm_melt = inv([wad, lm, Mg2SiO4L], [1.0, 2.0, 1.0])
...
pressures = np.empty_like(temperatures)
for i, T in enumerate(temperatures):
    pressures[i] = eqm_T(T, [wad, lm], [1.0, 2.0])
...
pressures = np.empty_like(temperatures)
for i, T in enumerate(temperatures):
    pressures[i] = eqm_T(T, [ring, lm], [1.0, 2.0])
...
temperatures = np.empty_like(pressures)
for i, P in enumerate(pressures):
    temperatures[i] = eqm(P, [lm, Mg2SiO4L], [2.0, 1.0])
