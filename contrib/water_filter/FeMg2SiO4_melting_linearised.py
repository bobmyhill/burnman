...
wad_ring_lm = inv([wad, ring, lm], [1., 1., 1.])
wad_lm_melt = inv([wad, lm, Mg2SiO4L], [1., 1., 1.])
...
pressures = np.empty_like(temperatures)
for i, T in enumerate(temperatures):
    pressures[i] = eqm_T(T, [wad, lm], [1., 1.])
...
pressures = np.empty_like(temperatures)
for i, T in enumerate(temperatures):
    pressures[i] = eqm_T(T, [ring, lm], [1., 1.])
...
pressures = np.linspace(10.e9, wad_lm_melt[0]+2.e9, 21)
temperatures = np.empty_like(pressures)
for i, P in enumerate(pressures):
    temperatures[i] = eqm(P, [ring, Mg2SiO4L])

ax[0].plot(pressures/1.e9, temperatures, color='black', linestyle=':')
...
temperatures = np.empty_like(pressures)
for i, P in enumerate(pressures):
    temperatures[i] = eqm(P, [lm, Mg2SiO4L], [1., 1.])
