...
def evaluate(
    ...
    assert X_Mg2SiO4_solid + X_Fe2SiO4_solid >= 0.0
    # 1) Calculate reference component properties
    c_prps = [
        thermodynamic_properties(P, T, Mg2SiO4_params),
    ...
    V_xs_solid = solid_excess_volume(
        P, T, X_ol_solid, X_MgSiO3_solid, X_H2O_solid, phases, f_tr
    )
    V_xs_melt = melt_excess_volume(P, T, X_Mg2SiO4_melt, X_Fe2SiO4_melt, X_H2O_melt)

    moles_solid = (1.0 - porosity) / (
        (
    ...
    X_Mg2SiO4_bulk = X_Mg2SiO4_solid * (1.0 - f_melt) + X_Mg2SiO4_melt * f_melt
    X_Fe2SiO4_bulk = X_Fe2SiO4_solid * (1.0 - f_melt) + X_Fe2SiO4_melt * f_melt
    X_MgSiO3_bulk = X_MgSiO3_solid * (1.0 - f_melt)
    X_H2O_bulk = X_H2O_solid * (1.0 - f_melt) + X_H2O_melt * f_melt
    ...
    if len(phases) == 1:
        prps = one_phase_eqm(
            P, T, X_Mg2SiO4_bulk, X_Fe2SiO4_bulk, X_MgSiO3_bulk, X_H2O_bulk, phases[0]
        )
        prpsdP = one_phase_eqm(
            P + dP,
            T,
            X_Mg2SiO4_bulk,
            X_Fe2SiO4_bulk,
    ...
    V_solid_molar = (
        prps["X_Mg2SiO4_solid"] * c_prps[0]["V"]
        + prps["X_Fe2SiO4_solid"] * c_prps[1]["V"]
        + prps["X_MgSiO3_solid"] * c_prps[2]["V"]
        + prps["X_H2O_solid"] * c_prps[3]["V"]
    ) + prps["V_xs_solid"]

    V_solid = V_solid_molar * (1.0 - prps["molar_fraction_melt"])

    V_melt_molar = (
        prps["X_Mg2SiO4_melt"] * c_prps[0]["V"]
        + prps["X_Fe2SiO4_melt"] * c_prps[1]["V"]
        + prps["X_H2O_melt"] * c_prps[3]["V"]
    ) + prps["V_xs_melt"]
    ...
    M_solid_molar = (
        prps["X_Mg2SiO4_solid"] * c_prps[0]["molar_mass"]
    ...
    V_xs_bulk = prps["V_xs_melt"] * prps["molar_fraction_melt"] + prps["V_xs_solid"] * (
        1.0 - prps["molar_fraction_melt"]
    )

    S_xs_bulk = prps["S_xs_melt"] * prps["molar_fraction_melt"] + prps["S_xs_solid"] * (
        1.0 - prps["molar_fraction_melt"]
    )

    dVdP_xs_bulk = (
        (
            prpsdP["V_xs_melt"] * prpsdP["molar_fraction_melt"]
            + prpsdP["V_xs_solid"] * (1.0 - prpsdP["molar_fraction_melt"])
        )
        - V_xs_bulk
    ) / dP
    ...
    dVdT_xs_bulk = (
        (
            prpsdT["V_xs_melt"] * prpsdT["molar_fraction_melt"]
            + prpsdT["V_xs_solid"] * (1.0 - prpsdT["molar_fraction_melt"])
        )
        - V_xs_bulk
    ) / dT
    ...
    dSdT_xs_bulk = (
        (
            prpsdT["S_xs_melt"] * prpsdT["molar_fraction_melt"]
            + prpsdT["S_xs_solid"] * (1.0 - prpsdT["molar_fraction_melt"])
        )
        - S_xs_bulk
    ) / dT

    V_bulk = (
        X_Mg2SiO4_bulk * c_prps[0]["V"]
        + X_Fe2SiO4_bulk * c_prps[1]["V"]
        + X_MgSiO3_bulk * c_prps[2]["V"]
        + X_H2O_bulk * c_prps[3]["V"]
    ) + V_xs_bulk

    S_bulk = (
        X_Mg2SiO4_bulk * c_prps[0]["S"]
        + X_Fe2SiO4_bulk * c_prps[1]["S"]
        + X_MgSiO3_bulk * c_prps[2]["S"]
        + X_H2O_bulk * c_prps[3]["S"]
    ) + S_xs_bulk

    dVdP_bulk = (
        X_Mg2SiO4_bulk * c_prps[0]["dVdP"]
        + X_Fe2SiO4_bulk * c_prps[1]["dVdP"]
        + X_MgSiO3_bulk * c_prps[2]["dVdP"]
        + X_H2O_bulk * c_prps[3]["dVdP"]
    ) + dVdP_xs_bulk

    dVdT_bulk = (
        X_Mg2SiO4_bulk * c_prps[0]["dVdT"]
        + X_Fe2SiO4_bulk * c_prps[1]["dVdT"]
        + X_MgSiO3_bulk * c_prps[2]["dVdT"]
        + X_H2O_bulk * c_prps[3]["dVdT"]
    ) + dVdT_xs_bulk

    dSdT_bulk = (
        X_Mg2SiO4_bulk * c_prps[0]["dSdT"]
        + X_Fe2SiO4_bulk * c_prps[1]["dSdT"]
        + X_MgSiO3_bulk * c_prps[2]["dSdT"]
        + X_H2O_bulk * c_prps[3]["dSdT"]
    ) + dSdT_xs_bulk

    M_bulk = (
        X_Mg2SiO4_bulk * c_prps[0]["molar_mass"]
...
if __name__ == "__main__":
    ...
    pressures = np.linspace(6.0e9, 30.0e9, 1001)
    # pressures = np.linspace(6.e9, 10.e9, 11)
    ...
    T = 2200.0
    for T in np.linspace(1400.0, 2000.0, 4):
        c = [0.65, 0.1, 0.2, 0.05]
        for i, P in enumerate(pressures):
            ...
            V_solid_molar = (
                eqm["X_Mg2SiO4_solid"] * c_prps[0]["V"]
                + eqm["X_Fe2SiO4_solid"] * c_prps[1]["V"]
                + eqm["X_MgSiO3_solid"] * c_prps[2]["V"]
                + eqm["X_H2O_solid"] * c_prps[3]["V"]
            ) + eqm["V_xs_solid"]

            V_solid = V_solid_molar * (1.0 - eqm["molar_fraction_melt"])

            V_melt_molar = (
                eqm["X_Mg2SiO4_melt"] * c_prps[0]["V"]
                + eqm["X_Fe2SiO4_melt"] * c_prps[1]["V"]
                + eqm["X_H2O_melt"] * c_prps[3]["V"]
            ) + eqm["V_xs_melt"]
            ...
            assert np.abs(out["porosity"] - old_porosity) < 1.0e-10
            assert np.abs(out["X_Mg2SiO4_solid"] - eqm["X_Mg2SiO4_solid"]) < 1.0e-10
            assert np.abs(out["X_Fe2SiO4_solid"] - eqm["X_Fe2SiO4_solid"]) < 1.0e-10
            assert np.abs(out["X_MgSiO3_solid"] - eqm["X_MgSiO3_solid"]) < 1.0e-10
            assert np.abs(out["X_H2O_solid"] - eqm["X_H2O_solid"]) < 1.0e-10
            assert np.abs(out["X_H2O_melt"] - eqm["X_H2O_melt"]) < 1.0e-10
            ...
            KD[i] = (
                2.0
                * out["X_Fe2SiO4_solid"]
                * (2.0 * out["X_Mg2SiO4_melt"] + out["X_MgSiO3_melt"])
                / (
                    2.0
                    * out["X_Fe2SiO4_melt"]
                    * (2.0 * out["X_Mg2SiO4_solid"] + out["X_MgSiO3_solid"])
                )
            )
        ...
        ax[1].plot(pressures / 1.0e9, solid_density, label="solid")
        ax[1].plot(pressures / 1.0e9, melt_density, label="melt")
        ax[1].plot(pressures / 1.0e9, bulk_density, label="bulk")
        ax[2].plot(pressures / 1.0e9, bulk_entropy, label="bulk")

        ax[3].plot(pressures / 1.0e9, bulk_alpha, label="bulk alpha")
        ax[4].plot(pressures / 1.0e9, bulk_beta, label="bulk beta")
        ax[5].plot(pressures / 1.0e9, bulk_Cp, label="bulk Cp")
        ...
        ax[1].set_ylim(2000.0, 4500.0)
        ax[2].set_ylim(2000.0, 3500.0)
        ax[3].set_ylim(-0.002, 0.0001)
