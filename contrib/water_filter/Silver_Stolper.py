...
for phase, expts in experiments.items():
    plt.scatter(expts[2]/(expts[1] + expts[2]), expts[0]+273.15, label=phase) # on a 1-cation basis

temperatures = np.linspace(100., Tm, 101)
a_H2O = np.empty_like(temperatures)
X_H2O = np.empty_like(temperatures)
for i, T in enumerate(temperatures):
    a_H2O[i], X_H2O[i] = ax_melt(T, Tm, Delta_Sf)
