...
def compositions(P, T, X_Mg2SiO4, X_H2O, X_MgSiO3, Tm,
    ...
    p_H2MgSiO4fo = a_H2OL*np.exp(-(gibbs(H2MgSiO4_polymorph, P, T)
    ...
    x_L = (X_H2O*(p_H2MgSiO4fo - 1) + X_Mg2SiO4*p_H2MgSiO4fo)/(p_H2MgSiO4fo
                                                               - p_H2OL)
...
# Olivine
plt.plot(fo_wtpctH2Os, temperatures-273.15,
         label=f'model melt ({P1/1.e9} GPa)', color='green', linestyle=':')
plt.plot(fo_wtpctH2Osb, temperatures-273.15,
         label=f'model melt ({P1b/1.e9} GPa)', color='green')

plt.plot(foL_wtpctH2Os, temperatures-273.15, color='green', linestyle=':')
plt.plot(foL_wtpctH2Osb, temperatures-273.15, color='green')

Mdata = np.genfromtxt('data/Mosenfelder_et_al_2006_ol_fluid.dat',
                      dtype=[(np.unicode_, 16), float, float, float,
                             (np.unicode_, 16), (np.unicode_, 16),
                             (np.unicode_, 16), float])
d = np.array([[d[1], d[2], d[-1]] for d in Mdata if d[-2] == 'yes' and d[1] == 8]).T
plt.errorbar(d[2]/10000, d[1], xerr=d[2]/10000/10, linestyle='None',
             color='green')
plt.scatter(d[2]/10000, d[1], label='Olivine (Mosenfelder et al., 2006)',
            color='green')

Mdata = np.genfromtxt('data/Smyth_et_al_2006_ol_melt.dat',
                      dtype=[(np.unicode_, 16), float, float, float])
d = np.array([[d[1], d[2], d[3]] for d in Mdata]).T
plt.errorbar(d[2]/10000, d[1], xerr=d[2]/10000/10, linestyle='None',
             color='green')
plt.scatter(d[2]/10000, d[1], label='Olivine (Smyth et al., 2006)',
            marker='P', color='green')

# Wadsleyite

plt.plot(wad_wtpctH2Os, temperatures-273.15,
         label=f'model melt ({P2/1.e9} GPa)', color='orange', linestyle=':')
plt.plot(wad_wtpctH2Osb, temperatures-273.15,
         label=f'model melt ({P2b/1.e9} GPa)', color='orange')
...
Ddata = np.genfromtxt('data/Demouchy_et_al_2005_wad_melt.dat',
                      dtype=[(np.unicode_, 16), float, float,
                             float, float, float])
d = np.array([list(d)[1:] for d in Ddata]).T
plt.errorbar(d[3], d[1], xerr=d[4], linestyle='None', color='orange')
plt.scatter(d[3], d[1], label='Wadsleyite (Demouchy et al., 2005)',
            marker='P', color='orange')

Ldata = np.genfromtxt('data/Litasov_et_al_2011_wad_melt.dat',
                      dtype=[(np.unicode_, 16), float, float, float,
                             float, float, float])
d = np.array([list(d)[1:] for d in Ldata]).T
plt.errorbar(d[2], d[1], xerr=d[3], linestyle='None', color='orange')
plt.scatter(d[2], d[1], label='Wadsleyite (Litasov et al., 2011)',
            color='orange')

# Ringwoodite
plt.plot(ring_wtpctH2Os, temperatures-273.15,
         label=f'model melt ({P3/1.e9} GPa)', color='blue')
plt.plot(ringL_wtpctH2Os, temperatures-273.15, color='blue')

Odata = np.genfromtxt('data/Ohtani_et_al_2000_rw_melt.dat',
...
plt.scatter(d[2], d[1], label='Ringwoodite (Ohtani et al., 2000)',
            color='blue')
