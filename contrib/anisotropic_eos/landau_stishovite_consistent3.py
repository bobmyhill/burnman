...
for i, P in enumerate(pressures):
    ...
    F1 = np.diag(sol1.x[1:] + 1.)
    V1 = (stv.V-dV/2.)*np.linalg.det(F1)
    sol2 = minimize(fn_helmholtz, guess, args=(stv.V+dV/2.))
    F2 = np.diag(sol2.x[1:] + 1.)
    V2 = (stv.V+dV/2.)*np.linalg.det(F2)

    new_pressures[i] = P - (sol2.fun - sol1.fun)/(V2 - V1)
    ...
    Cijs[i] = get_Cijs((V1+V2)/2., epsilon_voigt, Q+0.1)

    helmholtz[i] = stv.helmholtz + delta_helmholtz
    gibbs[i] = stv.helmholtz + delta_helmholtz + new_pressures[i] * (V1+V2)/2.
    volumes[i] = (V1+V2)/2.
