...
def make_voigt_matrix_inverse(Cs):
    ...
    print(T, V, f)
    if T < 400.:
        return np.array([T, P, V, betaT, betaT2])
    else:
        return np.array([np.nan, np.nan, np.nan, np.nan, np.nan])

dpsidf_data = np.empty((per_data.shape[0], 5))
for i in range(len(per_data)):
    dpsidf_data[i] = make_voigt_matrix_inverse(per_data[i])


#plt.plot(dpsidf_data[:,1], dpsidf_data[:,3])
#plt.plot(dpsidf_data[:,1], dpsidf_data[:,4])
plt.plot(dpsidf_data[:,1], dpsidf_data[:,2])
plt.show()
...
for idx in range(4):  # only plot 300 K data
    ...
    f2 = dpsidf_data[idx*n_d:(idx+1)*n_d, 3]
    d = dpsidf_data[idx*n_d:(idx+1)*n_d, 4:].T
    psi = cumulative_trapezoid(d, f, initial=0)
    print(f)
    for j in range(3):
        print(d[j, 0])
        ln, = ax[j].plot(f, psi[j] - psi[j,-1], label=f'{T} K')
        #ln, = ax[j].plot(f, d[j], label=f'{T} K')
