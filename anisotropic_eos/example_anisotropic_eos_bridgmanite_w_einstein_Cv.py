...
for P in [200.e9]:
    C_p, C_v = bdg_SLB.evaluate(['C_p', 'C_v'], temperatures*0. + P, temperatures)
    plt.plot(temperatures, C_p - C_v, label='{0}'.format(P/1.e9))

#Cvs = [eos.debye.molar_heat_capacity_v(T, 1460., 5.) for T in temperatures]
plt.plot(temperatures, 1.45*P_th(temperatures, 900.), linestyle=':')
plt.plot(temperatures, 1.45/2.*P_th2(temperatures, 900./4.), linestyle='--')
plt.legend()
plt.show()
exit()
"""
if __name__ == "__main__":
    ...
    K_V = []
    for datum in data:
        T, V, rho, P, C11T, C11S, C22T, C22S, C33T, C33S, C12T, C12S, C13T, C13S, C23T, C23S, C44, C55, C66  = datum
    ...
    """
    for temp in [0, 1500, 2500, 3500]:
        mask = [i for i, T in enumerate(data[:,0]) if T == temp]
        plt.fill_between(data[:,3][mask], np.array(K_R)[mask], np.array(K_V)[mask])
    plt.show()
    exit()
    """


    def fn_invcompliance(PT, invS0, invSprime, invSprime_inf, invSdprime, dinvSdT_0):
        theta, T0 = (905.*0.806, 300.)
        a, b, c, d, zeta, _, _ = eos.aeos.Sijkl_derivs_to_coeffs([invS0, invSprime,
                                                                  invSprime_inf,
                                                                  invSdprime,
                                                                  dinvSdT_0,
                                                                  theta, T0])
        ...
        return 1./eos.aeos.S_ijkl(P, Tmod, a, b, c, d, zeta, theta, T0)


    PT = np.array([data[:,3], data[:,0]]).T
    print(PT)
    S_prms = []
    for Sij in S_sets:
    ...
    for i in range(9):
        plt.plot(pressures, fn_invcompliance(PT, *S_prms[i,:5]), label=labels[i])

    for i in range(9):
        plt.scatter(data[:,3], 1./S_sets[i])
    plt.legend()
    plt.show()
