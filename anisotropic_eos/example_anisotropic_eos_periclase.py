...
if __name__ == "__main__":
    ...
    FP, FT, _, rho, _, VS100, _, VP100, _ = Fan_data.T[:9]
    VS110, _, VP110, _, CN11, _, CN12, _, CN44, _ = Fan_data.T[9:]
    ...
    S_11 = {'invS_0': 251.3e9,
            ...
            'grueneisen_0': 1.7*1.36127,
            'q_0': 1.}

    S_44 = {'invS_0': 153.8e9,
            'invSprime_0': 0.93,  # 2., 2. required for G_R~G_V at HP
            'invSprime_inf': 0.0,
    ...
    for T in [300., 1000., 1500.]:
        ...
        V, K_T = periclase.evaluate(['V', 'isothermal_bulk_modulus_reuss'],
                                    pressures, temperatures)
        plt.plot(pressures, K_T)
        plt.plot(pressures, -V*np.gradient(pressures, V, edge_order=2),
                 linestyle=':')
    plt.show()
    ...
    for P in [0., 25.e9, 50.e9, 100.e9, 200.e9]:
        for i, T in enumerate(temperatures):
            Pths[i] = eos.aeos.thermal_properties_ijkl(P, T, a, b, c, d, gr_0, q_0,
                                                       params)['P_th']
    ...
    for T in [300., 1000., 2000.]:
        ...
        volumes = periclase.evaluate(['isothermal_bulk_modulus_reuss'],
    ...
    for i, T in enumerate([300., 900., 1500., 1800.]):
        ...
        K_SLB, G_SLB = per_SLB.evaluate(['adiabatic_bulk_modulus',
                                         'shear_modulus'],
                                        pressures, temperatures)
        K_HP = per_HP.evaluate(['adiabatic_bulk_modulus'],
                               pressures, temperatures)[0]
        ...
        K_vrh, G_vrh = periclase.evaluate(['isentropic_bulk_modulus_vrh',
                                           'shear_modulus_vrh'],
                                          pressures, temperatures)
        K_v, G_v = periclase.evaluate(['isentropic_bulk_modulus_voigt',
                                       'shear_modulus_voigt'],
                                      pressures, temperatures)
        K_r, G_r = periclase.evaluate(['isentropic_bulk_modulus_reuss',
                                       'shear_modulus_reuss'],
                                      pressures, temperatures)

        ax[0].fill_between(pressures/1.e9, K_r/1.e9, K_v/1.e9,
                           color=c, alpha=0.3)
        ax[0].fill_between(pressures/1.e9, G_r/1.e9, G_v/1.e9,
                           color=c, alpha=0.3)

        ax[0].plot(pressures/1.e9, K_vrh/1.e9, color=c, linewidth=1.0,
                   label='$K^S$ ({0} K)'.format(int(T)))
        ax[0].plot(pressures/1.e9, G_vrh/1.e9, color=c, linewidth=1.0,
                   linestyle='--', label='$G$ ({0} K)'.format(int(T)))
        ax[1].plot(pressures/1.e9, np.gradient(K_vrh, pressures, edge_order=2),
                   color=c, linewidth=1., label='$K^S$ ({0} K)'.format(int(T)))
        ax[1].plot(pressures/1.e9, np.gradient(G_vrh, pressures, edge_order=2),
                   color=c, linewidth=1., linestyle='--',
                   label='$G$ ({0} K)'.format(int(T)))
    ...
    for T in [300., 800., 1300.]:
        ...
        vp_slb, vs_slb, vphi_slb = per_SLB.evaluate(['p_wave_velocity',
                                                     'shear_wave_velocity',
                                                     'bulk_sound_velocity'],
                                                    pressures, temperatures)
    ...
    for i, T in enumerate([300., 1000., 2000., 3000., 4000.]):
        temperatures = T + 0.*pressures
        G_R, G_V, G_VRH = periclase.evaluate(['shear_modulus_reuss',
                                              'shear_modulus_voigt',
                                              'shear_modulus_vrh'],
                                             pressures, temperatures)
        K_R, K_V, K_VRH = periclase.evaluate(['isentropic_bulk_modulus_reuss',
        ...
        ax[1].fill_between(pressures/1e9, G_R/1.e9, G_V/1.e9,
                           color=c, alpha=0.3,
                           linewidth=0)
    ...
    for T in [300., 1000., 2000.]:
        temperatures = T + 0.*pressures
        alphas, volumes, KT = periclase.evaluate(['alpha', 'V',
                                                  'isothermal_bulk_modulus_reuss'], pressures, temperatures)
        alphas_HP, volumes_HP, KT_HP = per_HP.evaluate(['alpha', 'V', 'K_T'],
                                                       pressures, temperatures)
        alphas_SLB, volumes_SLB, KT_SLB = per_SLB.evaluate(['alpha',
                                                            'V', 'K_T'],
                                                           pressures,
                                                           temperatures)

        ax[0].plot(pressures/1.e9, volumes*1.e6, label='{0} K'.format(T))
        ax[0].plot(pressures/1.e9, volumes_HP*1.e6, linestyle='--',
                   label='{0} K (HP)'.format(T))
        ax[0].plot(pressures/1.e9, volumes_SLB*1.e6, linestyle=':',
                   label='{0} K (SLB)'.format(T))

        ax[1].plot(pressures/1.e9, np.gradient(KT, pressures, edge_order=2),
                   label='K_T @ {0} K'.format(T))
        ax[1].plot(pressures/1.e9, np.gradient(KT_HP, pressures, edge_order=2),
                   linestyle='--', label='K_T @ {0} K (HP)'.format(T))
        ax[1].plot(pressures/1.e9,
                   np.gradient(KT_SLB, pressures, edge_order=2),
                   linestyle=':', label='K_T @ {0} K (SLB)'.format(T))

        ax[2].plot(pressures/1.e9, alphas*1.e6, label='a @ {0} K'.format(T))
        ax[2].plot(pressures/1.e9, alphas_HP*1.e6, linestyle='--',
                   label='a @ {0} K (HP)'.format(T))
        ax[2].plot(pressures/1.e9, alphas_SLB*1.e6, linestyle=':',
                   label='a @ {0} K (SLB)'.format(T))
    ...
    ax[1].scatter(FP[mask], np.gradient(1./(3.*SN11[mask] + 6.*SN12[mask]),
                                        FP[mask], edge_order=2))
