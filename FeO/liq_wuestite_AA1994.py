...
class liq_FeO (burnman.Mineral):
    def __init__(self):
        ...
        rho_0 = 4634. # 4632 is extrapolated from Hara et al., 1988a, b, (BaO and CaO solutions) similar to Ji et al., 1997 (4700)
        V_0 = m/rho_0
        self.params = {
            'name': 'liquid FeO',
            'formula': formula,
            ...
            'E_0': -119021., # Fit to standard state data
...
class liq_FeO (burnman.Mineral):
    def __init__(self):
        ...
        self.params = {
            ...
            'theta': 2000., # ? (goes into potential term)
            'lmda': [0., 0., 0.], # [302.07*m, -325.23*m, 30.45*m], # ? (goes into potential term)
            'xi_0': 62., # ? (goes into potential term)
            'F': [1., 1.],
            'n': sum(formula.values()),
            'molar_mass': m}
        burnman.Mineral.__init__(self)
...
'''
# Density estimate from dTdP/Coughlin estimate for S, sort-of agrees with Mori and Suzuki, 1968, see also Kress and Carmichael, 1989; (V_FeO = 13.087)
class liq_FeO (burnman.Mineral):
    def __init__(self):
        formula='FeO'
        formula = dictionarize_formula(formula)
        m = formula_mass(formula, atomic_masses)
        rho_0 = 5000. # Coughlin-derived
        V_0 = m/rho_0
        self.params = {
            'name': 'liquid FeO',
            'formula': formula,
