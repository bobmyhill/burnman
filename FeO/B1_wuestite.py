...
class B1_wuestite (Mineral):
    def __init__(self):
        ...
        self.params = {
            ...
            'K_0': 1.52e+11 , #147 GPa from Hazen and Finger 1982;  179 GPa in good agreement with K_S of Sumino et al; 184.3 from Will et al., 1980 see review by Hazen and Jeanloz, 1984
            'Kprime_0': 4.0 , # Fixed (Stixrude fit suggests 4.93)
            'Debye_0': 460.0 ,
            'grueneisen_0': 1.15 ,
            'q_0': 1.5 , # 1.5
            'T_el': 1400., # 4000.
...
class B1_wuestite (Mineral):
    def __init__(self):
        ...
        self.params = {
            ...
            'Kprime_0': 4.9 , # Fixed (Stixrude fit suggests 4.93)
            'Debye_0': 460.0 ,
            'grueneisen_0': 1.40 ,
            'q_0': 1.5 , # 1.5
            'T_el': 1400., # 4000.
...
if __name__ == "__main__":
    ...
    PTs = [list(B1_EoS_data[0]*1.e9), list(B1_EoS_data[2])]
    Vs = list(B1_EoS_data[10]*1.e-6)
    V_sigmas = list(B1_EoS_data[11]*1.e-6)

    PTs[0].extend([1.e5, 1.e5, 1.e5])
    PTs[1].extend([298.15, 1223.15, 1323.15])
    Vs.extend([1.2239e-05, volume([1.0, 1223.15], *popt), volume([1.0, 1323.15], *popt)])
    V_sigmas.extend([1.e-9, 1.e-9, 1.e-9])
    plt.plot(np.array(PTs[0])/1.e9, np.array(Vs), marker='o', linestyle='None')
    ...
    plt.show()
    #popt, pcov = burnman.tools.fit_PVT_data(fper, ['K_0', 'Kprime_0', 'grueneisen_0'], PTs, Vs, V_sigmas)
    #print popt
    #exit()
    
    temperatures = np.linspace(1., 2800., 501)
