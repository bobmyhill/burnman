...
Tmelt_fo = fsolve(delta_gibbs, 2000., args=(pressure_fo, fo, liquid, 1./3., 1.))[0]
print Tmelt_fo

print liquid.S, liquid.S*3. - fo.S
...
for i, T in enumerate(temperatures_fo):
    compositions0_fo[i]=fsolve(solve_composition, 0.001, args=(T, pressure_fo, r, K0, fn0, fn0, fo, liquid, 1./3., 1.))
    compositionsinf_fo[i]=fsolve(solve_composition, 0.001, args=(T, pressure_fo, r, Kinf, fn0, fn0, fo, liquid, 1./3., 1.))
    compositions_fo[i]=fsolve(solve_composition, 0.001, args=(T, pressure_fo, r, K, Wsh(pressure_fo), Whs(pressure_fo), fo, liquid, 1./3., 1.))
...
plt.plot( compositions_fo, temperatures_fo, linewidth=1, label='fo, 12 GPa')

'''
plt.plot( compositionsinf_fo, temperatures_fo, linewidth=1, label='K=inf, fo, P=20 GPa')
plt.plot( compositions0_fo, temperatures_fo, linewidth=1, label='K=0, fo, P=20 GPa')
'''
'''
###################
...
studynames=['Litasov_fo', 'Smyth', 'Demouchy', 'Litasov']

wtH2O=18.02
wtfo=140.6931/3.
studies_mol = []
for j, study in enumerate([litasov_fo, smyth, demouchy, litasov]):
...
for i, temperature in enumerate(smyth[0]):
    smyth_adjusted_mol[i] = fsolve(solve_composition, 0.001, args=(temperature, pressure_fo, r, K, Wsh(pressure_fo), Whs(pressure_fo), fo, liquid, 1./3., 1.))[0]

#plt.plot( smyth_adjusted, smyth[0], marker='o', linestyle='none', label='Smyth fit')
...
print 'Dwad/melt, Dfo/melt, Dwad/fo'
print demouchy_D, smyth_D, demouchy_D/smyth_D
...
plt.plot( compositions_fo_solid, temperatures_fo, linewidth=1, label='Ringwoodite')
...
#####
deltaH_foP = 170.e3 # 50 + 12*10 is Zhao et al., 2004
deltaV_fo = 10.0e-6 # 10e-6 is Zhao et al., 2004
n_fo = 1.
c1673_fo = 0.023 # wt fraction at 1400 C if pure water
...
# Print Dwad/fo at different temperatures
pressure = 14.e9 # roughly 410 km depth
temperatures = np.linspace(1200., 2200., 11)
for temperature in temperatures:
    print pressure/1.e9, temperature,  (A_wad*np.exp(-(deltaH_wad + pressure*deltaV_wad)/(constants.gas_constant*temperature))) /  (A_fo*np.exp(-(deltaH_fo + pressure*deltaV_fo)/(constants.gas_constant*temperature)))
...
plt.plot( compositions_fo_solid, temperatures_fo, linewidth=1, label='Forsterite (from activity, 12 GPa)')
plt.plot( compositions_fo_solid_2, temperatures_fo, linewidth=1, label='Forsterite (from activity, 14 GPa)')
...
print 'Prefactors:', A_wad, A_fo

pressure = 14.0e9 # 14 GPa is ~520 km
temperatures_Dwadfo = np.linspace(1273.15, 2273.15, 21)
Dwadmelt = np.empty_like(temperatures_Dwadfo)
Dfomelt = np.empty_like(temperatures_Dwadfo)
Dwadfo = np.empty_like(temperatures_Dwadfo)
for i, temperature in enumerate(temperatures_Dwadfo):
    ...
    fo_weight_percent = A_fo*np.power(a_H2O*f_H2O, n_fo)*np.exp(-(deltaH_fo + pressure*deltaV_fo)/(constants.gas_constant*temperature))
    
    Dwadfo[i] = (A_wad*np.exp(-(deltaH_wad + pressure*deltaV_wad)/(constants.gas_constant*temperature))) /  (A_fo*np.exp(-(deltaH_fo + pressure*deltaV_fo)/(constants.gas_constant*temperature)))
    Dwadfo[i] =  wad_weight_percent / fo_weight_percent
    Dwadmelt[i] = wad_weight_percent / melt_weight_percent
    Dfomelt[i] = fo_weight_percent / melt_weight_percent
    
    
    print pressure/1.e9, "GPa,", temperature-273.15, "C", Dwadfo[i]

plt.plot(temperatures_Dwadfo-273.15, Dwadfo)
plt.plot(temperatures_Dwadfo-273.15, Dwadmelt*10.)
plt.plot(temperatures_Dwadfo-273.15, Dfomelt*10.)
...
data=[['-W1,grey,-', compositions0_wad, temperatures_wad],  # wad melt at 15 GPa
      ['-W1,grey,-', compositions0_fo, temperatures_fo], # fo melt at 12 GPa
      ...
      ['-W1,black,-', compositions_fo, temperatures_fo], # fo melt at 12 GPa
      ['-W1,red', compositions_wad_solid, temperatures_wad],
      ['-W1,black', compositions_fo_solid, temperatures_fo]]
outfiles.append(['wad_fo_models.xT', data])
...
# Partition coefficients for forsterite, wadsleyite and melt.
outfiles = []
data=[['-W1,black', temperatures_Dwadfo, Dwadfo]]
outfiles.append(['wad_fo_partitioning.TD', data])

data=[['-W1,red,-', temperatures_Dwadfo, Dwadmelt],
      ['-W1,black,-', temperatures_Dwadfo, Dfomelt]]
outfiles.append(['solid_melt_partitioning.TD', data])
